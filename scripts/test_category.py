...
df = pd.read_csv("...data/test_category/test_category.csv")
...
for tq_index in tqdm(range(len(sentences))):
        ...
        cat_scores = []
        aspect_list = eval(aspects[aspect_index])
        for asp in aspect_list:
            asp = asp.lower()
            subtoken_ids = tokenizer.convert_tokens_to_ids(tokenizer.tokenize(asp))
            indices = []
